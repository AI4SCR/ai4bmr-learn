...
ncv = NestedCV(
    metadata=dm.dataset.metadata, save_dir=save_dir, test_size=0.2, force=True, num_outer_cv=5, num_inner_cv=3
)
...
for outer_fold in range(ncv.num_outer_cv):
    ...
    best_param_idx = (
        results.groupby("params_idx")[[metric_for_best_model, "trainable_params"]]
        .mean()
    ...
    config = dict(
        timestamp=timestamp, cv_type="outer", outer_fold=outer_fold, inner_fold=None, params_idx=best_param_idx
    )
    wandb_init = WandbInitConfig(project="cv", tags=["cv"], config=config)

    dm.splits_path = outer_split_path
    results_ = abmil(
        datamodule=dm, model=model_cfg, trainer=trainer, wandb_init=wandb_init, monitor_metric_name="train_loss_epoch"
    )
