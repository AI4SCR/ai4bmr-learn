...
def abmil_cv(
    ...
    ncv = NestedCV(metadata=dm.dataset.metadata, save_dir=save_dir, test_size=sweep.test_size, force=False,
                   num_outer_cv=sweep.num_outer_cv, num_inner_cv=sweep.num_inner_cv)
    ...
    for outer_fold in range(ncv.num_outer_cv):
        ...
        for params_idx, params in enumerate(sweep.parameters):
            ...
            for inner_fold in range(ncv.num_inner_cv):
                ...
                config = dict(cv_id=cv_id, cv_type='inner', outer_fold=outer_fold, inner_fold=inner_fold, params_idx=params_idx)
                wandb_init.config.update(config)

                dm.splits_path = inner_split_path
                results_ = abmil(datamodule=dm, model=model_cfg, trainer=trainer, weighted=weighted, wandb_init=wandb_init, monitor_metric_name='train_loss_epoch')
                results_.update(config)
                inner_results.append(results_)

        # FIND BEST PARAM_IDX
        inner_results = pd.json_normalize(inner_results)
        best_param_idx = inner_results \
