...
def get_torchvision_model(model_name: str, weights_version="DEFAULT"):
    from torchvision.models import get_model, get_model_weights, list_models

    available_models = list_models()
    if model_name not in available_models:
        raise ValueError(
            f"{model_name} not in available models. See: `torchvision.models.list_models()`"
        )

    if weights_version is not None:
        weights_version = weights_version or "DEFAULT"
        weights = get_model_weights(model_name)[weights_version]
